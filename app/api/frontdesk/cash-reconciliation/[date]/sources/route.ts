...
import { cashReconciliationSources } from "../../../../../../db/cash-reconciliation";
...
export async function GET(_request: Request, { params }: { params: Promise<{ date: string }> }) {
  ...
  try {
    ...
    const result = await cashReconciliationSources(user.email, { propertyId, businessDate: decodeURIComponent(date) });
    return Response.json(result, { headers: { "cache-control": "private, no-store" } });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Cash source detail could not load." }, { status: 409 });
  }
}
