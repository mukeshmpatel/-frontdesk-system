...
import { attachMaintenanceEvidence } from "../../../../../../db/maintenance-repairs";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!env.MEDIA) return Response.json({ error: "Private media storage is not connected. AAIQ will not claim photo verification without protected evidence storage." }, { status: 503 });
  const { id } = await context.params, propertyId = (await cookies()).get("aaiq_active_property")?.value || null;
  let objectKey: string | null = null;
  try {
    const form = await request.formData(), file = form.get("file");
    if (!(file instanceof File)) return Response.json({ error: "A Maintenance evidence image is required." }, { status: 400 });
    if (!file.type.startsWith("image/")) return Response.json({ error: "Maintenance evidence must be an image." }, { status: 415 });
    if (file.size > 15 * 1024 * 1024) return Response.json({ error: "Maintenance evidence must be 15MB or smaller." }, { status: 413 });
    const bytes = await file.arrayBuffer(), digest = await sha256Bytes(bytes);
    objectKey = `maintenance/evidence/${id}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
    await env.MEDIA.put(objectKey, bytes, { httpMetadata: { contentType: file.type || "application/octet-stream" } });
    const result = await attachMaintenanceEvidence(user.email, id, file, objectKey, digest,
